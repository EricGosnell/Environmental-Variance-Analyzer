...
module.exports = (db) => {
  ...
  router.put(
    ...
    async (req, res) => {
      try {
        ...
        await db.run("UPDATE users SET username = ? WHERE user_id = ?", [
          username,
          userId,
        ]);
  ...
  router.post(
    ...
    async (req, res) => {
      try {
        ...
        console.log(
          `[DEV] Verification code for ${newEmail}: ${verificationCode}`,
        );
  ...
  router.post(
    ...
    sanitizeRequestBody,
    [
      body("podId").trim().notEmpty().withMessage("Pod ID is required"),
  ...
  router.delete(
    ...
    async (req, res) => {
      try {
        ...
        await db.run("DELETE FROM user_pod WHERE user_id = ? AND pod_id = ?", [
          userId,
          podId,
        ]);
        ...
        return res
          .status(200)
          .json({ message: "Pod unregistered successfully" });
      } catch (error) {
        return res.status(500).json({
          error: "Internal server error",
          message: error?.message,
