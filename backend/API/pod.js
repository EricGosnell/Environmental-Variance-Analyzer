...
module.exports = (db) => {
      ...
      try {
        ...
        for (const pd of podDataRows) {
          ...
          for (const s of sensors) {
            const readingTimestampIso = new Date(
              s.reading_timestamp,
            ).toISOString();
            data.push({
  ...
  router.post(
    ...
    upload.single("file"),
    authenticateToken,
    // Request parameters: podId (string), data (file), notes (string) (optional)
    async (req, res) => {
      try {
        console.log("[DEBUG] /pods/upload-pod-data route hit");
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
        ...
        try {
          ...
          let ndjsonObjects;
          try {
            ndjsonObjects = await parseNDJSONStream(stream);
            console.log(
              "[DEBUG] NDJSON parsed:",
              ndjsonObjects.length,
              "objects",
            );
          } catch (parseErr) {
            console.error("[DEBUG] NDJSON parsing error:", parseErr);
            return res.status(400).json({
              error: "NDJSON parsing error",
              details: parseErr.message,
            });
          }

          for (const obj of ndjsonObjects) {
            const { pod_id, ts, seq, readings } = obj;
            if (!readings || !Array.isArray(readings)) {
              console.log(
                "[DEBUG] Missing or invalid readings in NDJSON object:",
                obj,
              );
              continue;
            }
            for (const reading of readings) {
              ...
              try {
                await db.run(sensorDataSQLString, [
                  pod_id,
                  metric,
                  value,
                  unit,
          ...
          return res
            .status(201)
            .json({ message: "Sensor data inserted successfully" });
        } catch (err) {
        ...
        console.error("[DEBUG] Route catch error:", error);
        return res.status(500).json({
          error: "Internal server error",
